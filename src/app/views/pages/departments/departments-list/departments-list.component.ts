...
export class DepartmentsListComponent implements OnInit {
  ...
  async remove(row: any): Promise<void> {
    const id = row.id || row._id;
    if (!id) return;
    const name = row.name || row.title || 'this department';
    const confirmed = await this.confirmService.confirm({
      title: 'Delete Department',
      message: 'Are you sure you want to delete this department? This can usually be restored from the API if soft-delete is enabled.',
      itemName: `${name}`,
